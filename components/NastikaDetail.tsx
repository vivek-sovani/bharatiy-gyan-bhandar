...
import { NASTIKA_DETAILS as DETAILS_EN } from '@/lib/nastika-data';
import { NASTIKA_DETAILS as DETAILS_MR } from '@/lib/nastika-data_mr';
import { useLanguage } from '@/lib/LanguageContext';

export default function NastikaDetail({ id }: { id: string }) {
  ...
  if (!data) {
    return (
      <section className="sec-hero">
        <div className="shell">
          <h2>Nāstika school not found</h2>
        </div>
      </section>
    );
  }

  // Get other Nastika schools for cross-navigation
  const allNastikas = Object.values(DETAILS);
  const otherNastikas = allNastikas.filter((n) => n.id !== id);
    ...
    <>
        ...
        <div className="shell sec-hero-inner">
          <div className="sec-hero-copy">
            <div className="sec-crumb">
              ...
              <Link href="/nastika-darshanas/">{lang === 'mr' ? 'नास्तिक दर्शने' : 'The Nāstika Darśanas'}</Link>
              <span className="sep">→</span>
          ...
          <div className="sec-hero-img-wrap" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'var(--paper-deep)' }}>
            <img
              src={`${process.env.NEXT_PUBLIC_BASE_PATH || ''}/corpus-nastika-darshanas.png`}
              alt={data.title}
              fetchPriority="high"
              style={{
                width: '100%',
          ...
          <div className="sec-panel">
            <div className="sec-panel-l">
              <div className="eyebrow" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                <Glyph /> {lang === 'mr' ? 'दर्शन परिचय' : 'School Profile'}
              </div>
              
              <dl className="sec-kv" style={{ marginTop: '1.5rem', marginBottom: '2rem' }}>
                <div className="sec-kv-row">
                  <dt>{lang === 'mr' ? 'प्रणेते / संस्थापक' : 'Founder/Key Sage'}</dt>
                  <dd>{data.founder}</dd>
                </div>
                <div className="sec-kv-row">
                  <dt>{lang === 'mr' ? 'प्रामाणिक ग्रंथ' : 'Key Literature'}</dt>
                  <dd>{data.texts}</dd>
                </div>
                <div className="sec-kv-row">
        ...
        <div className="shell">
          <div className="frame-hd">
            <div className="title-block">
              <div className="eyebrow"><Glyph /> {lang === 'mr' ? 'संप्रदाय व शाखा' : 'Sub-schools & Traditions'}</div>
              <h2>{lang === 'mr' ? `${data.title} मधील मुख्य संप्रदाय` : `Key Sub-schools of ${data.title}`}</h2>
            </div>
          </div>
      ...
      <section className="frame">
        <div className="shell">
          <div className="frame-hd">
            <div className="title-block">
              <div className="eyebrow"><Glyph /> {lang === 'mr' ? 'प्रमुख सिद्धांत' : 'Key Principles'}</div>
      ...
      <section className="see-also" style={{ background: 'var(--paper-deep)' }}>
        <div className="shell">
          <h4>{lang === 'mr' ? 'इतर नास्तिक दर्शने पहा' : 'Explore Other Nāstika Darśanas'}</h4>
          <div className="see-grid">
            {otherNastikas.map((n) => (
              <Link key={n.id} className="see-link" href={`/nastika-darshanas/${n.id}/`}>
                <span>
                  <span className="ttl">{n.title}</span>
                  <br />
                  <span className="ttl-de deva-only" style={{ color: 'var(--maroon)', fontSize: '0.92rem' }}>{n.deva}</span>
                </span>
