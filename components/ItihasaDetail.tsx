...
import { ITIHASA_DETAILS as DETAILS_EN } from '@/lib/itihasa-data';
import { ITIHASA_DETAILS as DETAILS_MR } from '@/lib/itihasa-data_mr';
import { useLanguage } from '@/lib/LanguageContext';

export default function ItihasaDetail({ id }: { id: string }) {
  ...
  // Get other epics for cross-navigation
  const allEpics = Object.values(DETAILS);
  const otherEpics = allEpics.filter((e) => e.id !== id);
    ...
    <>
        ...
        <div className="shell sec-hero-inner">
          <div className="sec-hero-copy">
            <div className="sec-crumb">
              ...
              <Link href="/itihasa/">{lang === 'mr' ? 'इतिहास ग्रंथ' : 'Itihāsa'}</Link>
              <span className="sep">→</span>
            ...
            <div className="epithet" style={{ fontSize: '1.25rem', marginTop: '0.5rem', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              <span>{lang === 'mr' ? 'कर्ते ऋषी: ' : 'Author Sage: '}</span>
              <span style={{ fontFamily: 'var(--font-deva)', color: 'var(--maroon)' }}>{data.author}</span>
            </div>
          ...
          <div className="sec-hero-img-wrap" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'var(--paper-deep)' }}>
            <img
              src={`${process.env.NEXT_PUBLIC_BASE_PATH || ''}/corpus-itihasa.png`}
              alt={data.title}
              fetchPriority="high"
              style={{
                width: '100%',
        ...
        <div className="shell sec-panel">
          <div className="sec-panel-l">
            <div className="eyebrow" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <Glyph /> {lang === 'mr' ? 'ग्रंथ परिचय' : 'Epic Profile'}
            </div>
            
            <dl className="sec-kv" style={{ marginTop: '1.5rem', marginBottom: '2rem' }}>
              <div className="sec-kv-row">
                <dt>{lang === 'mr' ? 'रचनाकार ऋषी' : 'Author'}</dt>
                <dd>{data.author}</dd>
              </div>
              <div className="sec-kv-row">
                <dt>{lang === 'mr' ? 'एकूण श्लोक संख्या' : 'Verse Count'}</dt>
                <dd>{data.verses}</dd>
              </div>
              <div className="sec-kv-row">
                <dt>{lang === 'mr' ? 'रचना विभाग' : 'Structure'}</dt>
                <dd>{data.structure}</dd>
              </div>
              <div className="sec-kv-row">
                <dt>{lang === 'mr' ? 'मुख्य सूत्र / संदेश' : 'Central Message'}</dt>
                <dd>{data.message}</dd>
              </div>
            </dl>
        ...
        <div className="shell">
          <div className="frame-hd">
            <div className="title-block">
              <div className="eyebrow"><Glyph /> {lang === 'mr' ? 'मुख्य विभाग व रचना' : 'Structure & Key Sections'}</div>
              <h2>{lang === 'mr' ? `${data.title} मधील प्रमुख विभाग` : `Key Books of ${data.title}`}</h2>
            </div>
          </div>
      ...
      <section className="frame">
        <div className="shell">
          <div className="frame-hd">
            <div className="title-block">
              <div className="eyebrow"><Glyph /> {lang === 'mr' ? 'प्रमुख सिद्धांत संकल्पना' : 'Key Ethical Concepts'}</div>
              <h2>{lang === 'mr' ? 'महाकाव्याचे मुख्य सिद्धांत' : 'Core Philosophical & Moral Tenets'}</h2>
            </div>
          </div>
      ...
      <section className="see-also" style={{ background: 'var(--paper-deep)' }}>
        <div className="shell">
          <h4>{lang === 'mr' ? 'इतर इतिहास ग्रंथ पहा' : 'Explore Other Epics'}</h4>
          <div className="see-grid">
            {otherEpics.map((e) => (
              <Link key={e.id} className="see-link" href={`/itihasa/${e.id}/`}>
                <span>
                  <span className="ttl">{e.title}</span>
                  <br />
                  <span className="ttl-de deva-only" style={{ color: 'var(--maroon)', fontSize: '0.92rem' }}>{e.deva}</span>
                </span>
