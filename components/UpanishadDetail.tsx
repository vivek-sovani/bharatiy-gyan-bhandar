...
export default function UpanishadDetail({ id }: { id: string }) {
  ...
  if (!data) {
    return (
      <section className="sec-hero">
        <div className="shell">
          <h2>Upaniṣad not found</h2>
        </div>
      </section>
    );
  }

  // Get other Upanishads for cross-navigation
  const allUpanishads = Object.values(DETAILS);
  const otherUpanishads = allUpanishads.filter((u) => u.id !== id);
    ...
    <>
        ...
        <div className="shell sec-hero-inner">
          <div className="sec-hero-copy">
            <div className="sec-crumb">
              ...
              <Link href="/upanishads/">{lang === 'mr' ? 'मुख्य उपनिषदे' : 'The Upaniṣads'}</Link>
              <span className="sep">→</span>
            ...
            <div className="epithet" style={{ fontSize: '1.25rem', marginTop: '0.5rem', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              <span>{lang === 'mr' ? 'संबंधित मुख्य वेद: ' : 'Veda Association: '}</span>
              <span style={{ fontFamily: 'var(--font-deva)', color: 'var(--maroon)' }}>{data.vedaAssociation}</span>
            </div>
          ...
          <div className="sec-hero-img-wrap" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'var(--paper-deep)' }}>
            <img
              src={`${process.env.NEXT_PUBLIC_BASE_PATH || ''}/corpus-upanishads.png`}
              alt={data.title}
              fetchPriority="high"
              style={{
                width: '100%',
          ...
          <div className="sec-panel">
            <div className="sec-panel-l">
              <div className="eyebrow" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                <Glyph /> {lang === 'mr' ? 'उपनिषद परिचय' : 'Upaniṣad Profile'}
              </div>
              
              <dl className="sec-kv" style={{ marginTop: '1.5rem', marginBottom: '2rem' }}>
                <div className="sec-kv-row">
                  <dt>{lang === 'mr' ? 'वेद संबंध' : 'Associated Veda'}</dt>
                  <dd style={{ fontFamily: 'var(--font-deva)', color: 'var(--maroon)' }}>{data.vedaAssociation}</dd>
                </div>
                <div className="sec-kv-row">
                  <dt>{lang === 'mr' ? 'श्लोक / खंड संख्या' : 'Size / Verses'}</dt>
                  <dd>{data.versesCount}</dd>
                </div>
                <div className="sec-kv-row">
                  <dt>{lang === 'mr' ? 'मुख्य गाभा' : 'Core Focus'}</dt>
                  <dd>{data.focus}</dd>
                </div>
              </dl>
        ...
        <div className="shell">
          <div className="frame-hd">
            <div className="title-block">
              <div className="eyebrow"><Glyph /> {lang === 'mr' ? 'मुख्य विचार व संरचना' : 'Structure & Core Ideas'}</div>
              <h2>{lang === 'mr' ? `${data.title} मधील प्रमुख विषय` : `Key Themes of ${data.title}`}</h2>
            </div>
          </div>
      ...
      <section className="frame">
        <div className="shell">
          <div className="frame-hd">
            <div className="title-block">
              <div className="eyebrow"><Glyph /> {lang === 'mr' ? 'मूलभूत आध्यात्मिक संकल्पना' : 'Key Concepts'}</div>
      ...
      <section className="see-also" style={{ background: 'var(--paper-deep)' }}>
        <div className="shell">
          <h4>{lang === 'mr' ? 'इतर उपनिषदे पहा' : 'Explore Other Upaniṣads'}</h4>
          <div className="see-grid">
            {otherUpanishads.map((u) => (
              <Link key={u.id} className="see-link" href={`/upanishads/${u.id}/`}>
                <span>
                  <span className="ttl">{u.title}</span>
                  <br />
                  <span className="ttl-de deva-only" style={{ color: 'var(--maroon)', fontSize: '0.92rem' }}>{u.deva}</span>
                </span>
