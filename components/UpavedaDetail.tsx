...
import { UPAVEDAS_DETAILS as DETAILS_EN } from '@/lib/upavedas-data';
import { UPAVEDAS_DETAILS as DETAILS_MR } from '@/lib/upavedas-data_mr';
import { useLanguage } from '@/lib/LanguageContext';

export default function UpavedaDetail({ id }: { id: string }) {
  ...
  if (!data) {
    return (
      <section className="sec-hero">
        <div className="shell">
          <h2>Upaveda not found</h2>
        </div>
      </section>
    );
  }

  // Get other Upavedas for cross-navigation
  const allUpavedas = Object.values(DETAILS);
  const otherUpavedas = allUpavedas.filter((u) => u.id !== id);
  ...
  return (
    <>
        ...
        <div className="shell sec-hero-inner">
          <div className="sec-hero-copy">
            <div className="sec-crumb">
              ...
              <Link href="/upavedas/">{lang === 'mr' ? 'चार उपवेद' : 'The Four Upavedas'}</Link>
              <span className="sep">→</span>
            ...
            <div className="epithet" style={{ fontSize: '1.25rem', marginTop: '0.5rem', display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              <span>{lang === 'mr' ? 'संबंधित मुख्य वेद: ' : 'Paired Principal Veda: '}</span>
              <span style={{ fontFamily: 'var(--font-deva)', color: 'var(--maroon)' }}>{data.vedaPair}</span>
            </div>
          ...
          <div className="sec-hero-img-wrap" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'var(--paper-deep)' }}>
            <img
              src={`${process.env.NEXT_PUBLIC_BASE_PATH || ''}/corpus-upavedas.png`}
              alt={data.title}
              fetchPriority="high"
              style={{
                width: '100%',
        ...
        <div className="shell">
          <div className="frame-hd">
            <div className="title-block">
              <div className="eyebrow"><Glyph /> {lang === 'mr' ? 'शाखा व अंग विभाग' : 'Branches & Disciplines'}</div>
              <h2>{lang === 'mr' ? `${data.title} चे प्रमुख पैलू` : `Key Aspects of ${data.title}`}</h2>
            </div>
          </div>
        ...
        <div className="shell">
          <div className="frame-hd">
            <div className="title-block">
              ...
              <h2>{lang === 'mr' ? 'महत्त्वाच्या संकल्पना व व्याख्या' : 'Core Philosophical & Technical Concepts'}</h2>
            </div>
          </div>
      ...
      <section className="see-also" style={{ background: 'var(--paper-deep)' }}>
        <div className="shell">
          <h4>{lang === 'mr' ? 'इतर उपवेद पहा' : 'Explore Other Upavedas'}</h4>
          <div className="see-grid">
            {otherUpavedas.map((u) => (
              <Link key={u.id} className="see-link" href={`/upavedas/${u.id}/`}>
                <span>
